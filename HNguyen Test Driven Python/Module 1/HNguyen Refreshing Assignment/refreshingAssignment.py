...
#setters
    def add_item(self, drink):
        if not isinstance(drink,Drink):
            print(f"{drink} is not a drink instance")
...
class Drink:
    ...
    def __init__(self,base,flavors):
        if not isinstance(flavors,list):
            print("flavors must be in a list")
            return
        self._base = base
    ...
    def set_flavors(self,flavorArray):
        for item in flavorArray:
            if item not in flavorChoices:
                print(f"flavor not in flavor choices: {item}")
                return
        if duplicateCheck(flavorArray):
            print(f"duplicate of {item} in given list")
            return
        self._flavors = flavorArray

    def add_flavors(self,flavor):
        if flavor in self._flavors:
            print(f"flavor already in list: {flavor}")
            return
...
drinker = Drink("water",["lemon"])
drinker.set_flavors(["lemon","strawberry","blueberry"])
drinker2 = Drink("sbrite",["lemon","lime"])
order1 = Order([drinker,drinker2])
print(order1.get_reciept())